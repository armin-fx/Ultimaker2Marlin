...
float current_position[NUM_AXIS] = { X_MIN_POS, Y_MIN_POS, Z_MIN_POS, 0.0 };
...
void filament_grab_init()
{
	for (int i=0; i<EXTRUDERS; ++i)
	{
		...
		filament_grab_value[i]    = 0;
	}
}

void filament_grab_update(float filament_lenght)
{
	if (!is_prevent_filament_grind()) // prevent filament grind is off
	{
		filament_grab_value[active_extruder] = 0;
		return;
	}
	else if (filament_lenght < 0) // retract
	{
...
#define FILAMENT_GRAB_VALUE_BEGIN  0.5

// Hold current retract length on preferred retract length until value has reached FILAMENT_GRAB_VALUE_BEGIN.
...
// value = 1: filament was grab per feeder (statistic) the adjusted filament_grab_max.
...
static void get_coordinates(const char *cmd)
{
    ...
    if (IS_AXIS_MOVED(seen, E_AXIS)) // e is moved
    {
        ...
        if(autoretract_enabled)
        {
            ...
            {
                filament_grab_update(echange * volume_to_filament_length[active_extruder]);
            }
            #endif
        }
    #ifdef PREVENT_FILAMENT_GRIND
