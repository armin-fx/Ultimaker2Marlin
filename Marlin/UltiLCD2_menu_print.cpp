        ...
        if (selIndex == 0)
        {
            ...
            if (!card.filenameIsDir())
            {
                ...
                if (card.isFileOpen() && !commands_queued())
                {
                    ...
                    card.truncateLongFilename((ui_mode & UI_SCROLL_ENTRY) ? LONG_FILENAME_LENGTH-1 : 20);
...
//                            SERIAL_ECHOLN(LCD_DETAIL_CACHE_MATERIAL(e));
                            ...
                            volume_to_filament_length[e] = 1.0 / (M_PI / 4.0 * material[e].diameter * material[e].diameter);
...
{
    LED_GLOW
    lcd_question_screen(lcd_menu_doabort, set_abort_state, MSGP_MENU_YES, NULL, lcd_change_to_previous_menu, MSGP_MENU_NO);
...
void lcd_menu_print_tune_heatup_nozzle0()
{
    ...
    {
        target_temperature[0] = constrain(int(target_temperature[0]) + (lcd_lib_encoder_pos / ENCODER_TICKS_PER_TUNE_VALUE_ITEM)
                                         , 0, get_maxtemp(0) - 15);
...
void lcd_menu_print_tune_heatup_nozzle1()
{
    if (lcd_lib_encoder_pos / ENCODER_TICKS_PER_TUNE_VALUE_ITEM != 0)
    {
        target_temperature[1] = constrain(int(target_temperature[1]) + (lcd_lib_encoder_pos / ENCODER_TICKS_PER_TUNE_VALUE_ITEM)
                                         , 0, get_maxtemp(1) - 15);
        lcd_lib_encoder_pos = 0;
    }
    if (lcd_lib_button_pressed)
...
void lcd_menu_print_tune()
{
    ...
    if (lcd_lib_button_pressed)
    {
        ...
        if (IS_SELECTED_SCROLL(index++))
            ...
            menu.add_menu(menu_t(lcd_menu_print_limits, 0));
        else if (IS_SELECTED_SCROLL(index++))
...
static void lcd_retraction_details(uint8_t nr)
{
    ...
    if(nr == item++)
        ...
        int_to_string(retract_feedrate / 60 + 0.5, buffer, MSGP_UNIT_SPEED2);
    else if(nr == item++)
...
static void lcd_menu_print_tune_retraction()
{
    ...
    if (lcd_lib_button_pressed)
    {
        ...
        if (IS_SELECTED_SCROLL(item++))
            ...
            LCD_EDIT_SETTING_SPEED_P(retract_feedrate, MSGP_RETRACT_SPEED, MSGP_UNIT_SPEED2, 0, max_feedrate[E_AXIS] * 60);
        else if (IS_SELECTED_SCROLL(item++))
...
static void lcd_limits_item(uint8_t nr, uint8_t offsetY, uint8_t flags)
{
    ...
    if (nr == item++)
        ...
        strcpy_P(buffer, PSTR("max print speed"));
    else if (nr == item++)
        strcpy_P(buffer, PSTR("max travel speed"));
#if EXTRUDERS > 1
    else if (nr == item++)
        strcpy_P(buffer, PSTR("max extrusion 1"));
    else if (nr == item++)
        strcpy_P(buffer, PSTR("max extrusion 2"));
#else
    else if (nr == item++)
